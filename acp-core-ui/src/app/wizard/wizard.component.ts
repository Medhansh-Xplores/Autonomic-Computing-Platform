...
export class WizardComponent implements OnInit, AfterViewInit {
  ...
  azureCpu: string = '0.5';
  azureMemory: string = '1';
  serverIdentifier: string = '';
  azureDbEngine: string = 'postgres';
  azureDbUsername: string = '';
  azureDbPassword: string = '';
  azureInitialDbName: string = '';
  createAzureInitialDb: boolean = false;
  azureClusterName: string = '';
  azureCostEstimate: any = null;
  azureCostLoading = false;
  azureCostError = '';
  ...
  @ViewChild('accountsContainerApps', { static: false }) accountsContainerApps!: ElementRef;
  @ViewChild('accountsAzureDB', { static: false }) accountsAzureDB!: ElementRef;
  @ViewChild('accountsAKS', { static: false }) accountsAKS!: ElementRef;
  ...
  ngAfterViewInit() {
    ...
    this.http.get<any>(this.apiBase + "tenants/" + this.licenseNum + "/accounts?platformTypeFilter=AZURE")
      .subscribe(data => {
        ...
        this.renderer.setProperty(this.accountsContainerApps.nativeElement, 'innerHTML', tempVal);
        this.renderer.setProperty(this.accountsAzureDB.nativeElement, 'innerHTML', tempVal);
        this.renderer.setProperty(this.accountsAKS.nativeElement, 'innerHTML', tempVal);
      });
  ...
  changePage(e: any) {
    ...
    if (changeNum == 0) {
      ...
      if (this.blueprintID == "aws-vpc") {
        ...
        changeNum = 3;
        this.blueprintDescription = "Azure Virtual Network (VNet)";
      }

      else if (this.blueprintID == "azure-container-apps") {
        changeNum = 11;
        this.blueprintDescription = "Azure Container Apps";
        this.containerAppName = this.zoneName;
      }

      else if (this.blueprintID == "azure-db") {
        changeNum = 13;
        this.blueprintDescription = "Azure SQL / Postgres Database";
        this.serverIdentifier = this.zoneName;
      }

      else if (this.blueprintID == "azure-aks") {
        changeNum = 15;
        this.blueprintDescription = "Azure Kubernetes Service (AKS)";
        this.azureClusterName = this.zoneName;
      }
    ...
    // AZURE CONTAINER APPS VALIDATION (step 11 -> 12)
    if (changeNum == 12 && this.blueprintID == "azure-container-apps") {
      if (!this.accountNum || !this.region || !this.containerAppName || !this.azureCpu || !this.azureMemory) {
        alert("Please fill all required fields");
        return;
      }

      this.loadAzureCostEstimate("container-apps");
    }

    // AZURE DB VALIDATION (step 13 -> 14)
    if (changeNum == 14 && this.blueprintID == "azure-db") {
      if (!this.accountNum || !this.region || !this.azureDbEngine || !this.serverIdentifier || !this.azureDbUsername || !this.azureDbPassword) {
        alert("Please fill all required fields");
        return;
      }

      this.loadAzureCostEstimate("db");
    }

    // AZURE AKS VALIDATION (step 15 -> 16)
    if (changeNum == 16 && this.blueprintID == "azure-aks") {
      if (!this.accountNum || !this.region || !this.azureClusterName) {
        alert("Please fill all required fields");
        return;
      }

      this.loadAzureCostEstimate("aks");
    }
  ...
  loadVpcs() {
    ...
    if (this.blueprintID !== "aws-ecs" && this.blueprintID !== "aws-rds") {
      return;
    }
  ...
  getAzureEstimatedMonthlyCost(): number | null {
    return this.azureCostEstimate?.estimatedMonthlyCost ?? null;
  }

  loadAzureCostEstimate(type: 'container-apps' | 'db' | 'aks') {
    this.azureCostLoading = true;
    this.azureCostError = '';
    this.azureCostEstimate = null;
    ...
    if (type === 'container-apps') {
      payload.cpu = Number(this.azureCpu);
      payload.memory = Number(this.azureMemory);
    }

    if (type === 'db') {
      payload.dbEngine = this.azureDbEngine;
    }

    if (type === 'aks') {
      payload.nodeCount = 1;
      payload.vmSize = 'Standard_B2s';
    }

    this.http.post<any>(this.apiBase + 'cost-estimates/azure', payload).subscribe({
      next: (estimate) => {
        this.azureCostEstimate = estimate;
        this.azureCostLoading = false;
      },
      error: (err) => {
        console.error(err);
        this.azureCostError = err?.error?.error || 'Unable to load Azure price estimate.';
        this.azureCostLoading = false;
      }
    });
  }
  ...
  submitForProcessing() {
    ...
    if (this.blueprintID == "azure-vpc") {
      const body = {
        tenantID: this.licenseNum,
        blueprintID: this.blueprintID,
        accountID: this.accountNum,
        region: this.region,
        vnetName: this.VPCname,
        cidr: this.CIDRBlock,
        zoneName: this.zoneName
      };
      this.isCreating = true;
      this.http.post<any>(this.apiBase + 'deployments/azure-vpc', body).subscribe({
        next: () => {
          this.isCreating = false;
          this.router.navigate(['/deployment-logs'], { state: { infraType: this.blueprintDescription } });
        },
        error: (err) => {
          console.error(err);
          this.isCreating = false;
          alert('Deployment failed: ' + (err.error?.error || err.message));
        }
      });
    }
    ...
    // ── AZURE Container Apps ────────────────────────────────
      ...
      this.isCreating = true;
      this.http.post<any>(this.apiBase + 'deployments/azure-container-apps', body).subscribe({
        next: () => {
          this.isCreating = false;
          this.router.navigate(['/deployment-logs'], { state: { infraType: this.blueprintDescription } });
        },
    ...
    // ── AZURE Database ───────────────────────────────────────────
      ...
      const body = {
        ...
        dbEngine: this.azureDbEngine,
        dbUsername: this.azureDbUsername,
        dbPassword: this.azureDbPassword,
        createInitialDb: this.createAzureInitialDb,
        initialDbName: this.azureInitialDbName,
        vnetName: this.VPCname,
        zoneName: this.zoneName
      };
      this.isCreating = true;
      this.http.post<any>(this.apiBase + 'deployments/azure-db', body).subscribe({
        next: () => {
          this.isCreating = false;
          this.router.navigate(['/deployment-logs'], { state: { infraType: this.blueprintDescription } });
        },
    ...
    // ── AZURE Kubernetes Service ─────────────────────────────────
      ...
      const body = {
        ...
        accountID: this.accountNum,
        region: this.region,
        clusterName: this.azureClusterName,
        vnetName: this.VPCname,
        zoneName: this.zoneName
      };
      this.isCreating = true;
      this.http.post<any>(this.apiBase + 'deployments/azure-aks', body).subscribe({
        next: () => {
          this.isCreating = false;
          this.router.navigate(['/deployment-logs'], { state: { infraType: this.blueprintDescription } });
        },
    ...
    // ── AWS VPC ──────────────────────────────────────────────────
    else if (this.blueprintID == "aws-vpc") {
      // AWS STANDARD
      // Auto generate AZs
