...
export class InfrastructureResourcesComponent implements OnInit, OnDestroy {
  ...
  getCloudName(deployment: any): string {
    const cloud = String(deployment?.cloud || '').toLowerCase();

    if (cloud === 'aws') return 'AWS';
    if (cloud === 'azure') return 'Azure';
    if (cloud === 'gcp') return 'GCP';

    const type = String(deployment?.type || '').toLowerCase();
    if (type.startsWith('azure-')) return 'Azure';
    if (['vpc', 'ecs', 'rds'].includes(type)) return 'AWS';

    return deployment?.cloud || 'Unknown';
  }

  getDestroyedBy(deployment: any): string {
    if (deployment?.destroyedBy || deployment?.deletedBy) {
      return deployment.destroyedBy || deployment.deletedBy;
    }

    return this.isDestroyed(deployment) ? 'Cloud Service' : '-';
  }

  isDestroyed(deployment: any): boolean {
  ...
  deleteDeployment(event: Event, deployment: any) {
    ...
    if (!confirm(`Delete ${deployment.name}? This will destroy it from ${this.getCloudName(deployment)}.`)) return;
    ...
    // The 5s auto-refresh will show the retained destroyed record.
  }
