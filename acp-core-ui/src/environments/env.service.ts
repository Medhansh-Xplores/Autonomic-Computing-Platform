...
export class EnvService {
  ...
  private setEnvVariables(): void {
    ...
    if (/^.*localhost.*/.test(hostname)) {
      ...
      // Running on ALB URL or any other host — use relative URL
      // so requests go to same host and ALB routes /api/* to backend
      this._env = Environment.Prod
      this._apiUrl = '/api/v1/'    // ← relative, works on any hostname
    }
  }
}